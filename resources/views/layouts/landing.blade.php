...
<head>
    ...
    <title>{{ config('app.name', 'FitStudio') }} - Supera tus límites</title>
...
<body class="antialiased bg-gray-50 text-gray-900" x-data="{ mobileMenuOpen: false, scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 50)">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex-shrink-0 z-50 relative">
                    <a href="#" class="text-white text-3xl font-extrabold tracking-wider transition-transform duration-300 hover:scale-105 inline-block">
                        FIT<span class="text-emerald-500">STUDIO</span>
                    </a>
                </div>
                ...
                <div class="hidden md:block">
                    <div class="ml-10 flex items-center space-x-8">
                        ...
                        <div class="relative group">
                            <div class="absolute -inset-0.5 bg-gradient-to-r from-emerald-500 to-teal-400 rounded-full blur opacity-50 group-hover:opacity-100 transition duration-500"></div>
                            <a href="/clientes/login" class="relative bg-black border border-white/20 text-white px-6 py-2.5 rounded-full text-sm font-bold transition-all duration-300 hover:bg-emerald-500 hover:border-emerald-500 flex items-center">
                                Iniciar Sesión
                ...
                <div class="md:hidden z-50 relative flex items-center">
                    <button @click="mobileMenuOpen = !mobileMenuOpen" class="text-white focus:outline-none w-10 h-10 flex flex-col justify-center items-center group">
                        <span :class="mobileMenuOpen ? 'rotate-45 translate-y-2.5 bg-emerald-500' : 'bg-white'" class="block w-8 h-0.5 rounded-full transition-all duration-300 ease-in-out"></span>
                        ...
                        <span :class="mobileMenuOpen ? '-rotate-45 -translate-y-2.5 bg-emerald-500' : 'bg-white'" class="block w-8 h-0.5 rounded-full mt-2 transition-all duration-300 ease-in-out"></span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <div x-show="mobileMenuOpen"
        ...
        <div class="flex flex-col items-center space-y-8 w-full px-6">
            <a href="#hero" @click="mobileMenuOpen = false" class="text-3xl font-bold text-white hover:text-emerald-500 transition-colors transform hover:scale-110 duration-300">Inicio</a>
            <a href="#clases" @click="mobileMenuOpen = false" class="text-3xl font-bold text-white hover:text-emerald-500 transition-colors transform hover:scale-110 duration-300">Clases</a>
            <a href="#testimonios" @click="mobileMenuOpen = false" class="text-3xl font-bold text-white hover:text-emerald-500 transition-colors transform hover:scale-110 duration-300">Testimonios</a>
            <a href="#ubicacion" @click="mobileMenuOpen = false" class="text-3xl font-bold text-white hover:text-emerald-500 transition-colors transform hover:scale-110 duration-300">Ubicación</a>

            <div class="w-full h-px bg-white/20 my-4"></div>

            <a href="/clientes/login" class="w-full text-center bg-emerald-500 hover:bg-emerald-600 text-white py-4 rounded-2xl text-xl font-bold transition shadow-[0_0_20px_rgba(16,185,129,0.4)]">
                Iniciar Sesión
            </a>
        </div>
    </div>
